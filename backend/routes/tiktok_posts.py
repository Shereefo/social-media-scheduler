...
async def create_tiktok_post(
    ...
    try:
        ...
        logger.info(f"Created TikTok post with ID: {post.id} for user: {current_user.username}")
...
async def publish_tiktok_post(
    ...
    try:
        ...
        # Publish to TikTok
        result = await TikTokAPI.post_video(db, current_user, video_content, post.content)
        ...
        return {"message": "Post published successfully", "tiktok_post_id": result["data"]["post_id"]}
    except Exception as e:
